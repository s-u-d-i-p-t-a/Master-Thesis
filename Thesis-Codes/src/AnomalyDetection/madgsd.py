...
class MADGSDAnamoly():
    """ 
    MADESDAnomaly objects generate anomaly measurements from input series
    ...
    def __init__(self, series, dates=None):
        ...
        """
        self.series = [np.array(x) for x in MADGSDAnamoly.standardize_data(series)]
        self.dates = dates
    ...
    def standardize_data(series):
        ...
        # can't select from dict with [0] index so need separate case
            ...
            for sub in series.values():
                lists.extend(MADGSDAnamoly.standardize_data(sub))
            return lists
        # standard use case
    ...
    def seasonal_esd(inputs, a=.025, frequency=3, start=None, end=None):
        """
        Static method that generates anomalies from the given series
            using S-H-ESD algorithm developed by twitter
        Note:
            Based on frequency, the decomposition trend values can become NULL near the bounds of the series
        ...
        for i in range(len(vals)):
            ...
            pairs.append((v, i))
        return MADGSDAnamoly.grubbs_test(inputs, pairs, start, end, a)
    ...
    def grubbs_test(inputs, errs, start=None, end=None, a=.025):
        ...
        while check == True:
            ...
            #s = np.nanstd(series)
            s = mad(series, c=0.6745, axis=0, center=u)    # find residual with largest z value, or distance from median
    ...
    def arima_test(values, clean, start=None, end=1, strict=True):
        ...
        try:
            ...
            return MADGSDAnamoly.grubbs_test(values, errs)
    ...
    def seasonality(y):
        ...
        """
        vals = MADGSDAnamoly.derivative(y)
    ...
    def detect_series(series, start, end, strict=False, a=0.025):
        ...
        """
        vals = np.copy(series)
        potential = MADGSDAnamoly.seasonal_esd(vals, a=a, frequency=MADGSDAnamoly.seasonality(vals), start=start, end=end)
        ...
        others = MADGSDAnamoly.arima_test(vals, clean)
        if strict:
            confirmed = []
    ...
    def detect(self, start=None, end=None, strict=False, a=0.025):
        """
        Finds anomalies in each series of MADGSDAnomaly
        Args:
            start (int): the first index in the series to check for anomalies
            end (int): the last index in the series to check for anomalies
        Returns:
             list or list of lists containing anomalies for MADGSDAnomaly
        """
        results = []
        ...
        for sub_series in self.series:
            ...
            results.append(MADGSDAnamoly.detect_series(sub_series, s, e, strict, a))
    ...
    def detect_latest(self, strict=False, a=0.025):
        """
        Finds last index anomalies in each series of MADGSDAnomaly
        Notes:
